...
|-----------|-----------|--------|
| Unit | pytest | 228 |
| Integration | pytest-django | 115 |
| E2E | pytest-django | 4 |
| **Gesamt** | | **347** |

> Stand 2026-07-22: **366 Tests grün** (`venv/bin/python3 -m pytest`, 0 Errors).
...
- Echte PostgreSQL-Datenbank (cmp_django_test)
...
# Mit Coverage
python -m pytest tests/ --cov=cmp --cov-report=term-missing
```
...
**testing.py:** Schnelle Passwort-Hashes (`MD5PasswordHasher`), Celery EAGER-Modus, Test-Datenbank `cmp_django_test`.

## Hinweis: keepdb

Die Test-DB wird mit `keepdb=True` wiederverwendet (cmp-User hat kein CREATEDB). Bei Schema-Änderungen muss die DB manuell gelöscht werden:
