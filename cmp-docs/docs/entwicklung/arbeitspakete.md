...
Status: **AP-0 … AP-10 fertig** (Backend B0–B9 + HTMX-Frontend, 330 Tests grün, v1.3.1), **AP-11 Container (Podman/Quadlets) offen** — Abwägung Native-vs-Container siehe [ADR-0001](../decisions/0001-deployment-native-vs-container.md) —, **AP-12 Live-Updates (Django Channels) offen** — Channels stand im v1-Zielbild, ist aber noch nicht gebaut (kein `channels` in den Requirements, `asgi.py` Django-Default); Benachrichtigungen aktualisieren bis dahin per Seiten-Reload.
Zusätzlich (nicht-AP):
...
- **Offline-Release für AlmaLinux 9** (gebündelte Wheels + `install.sh`, GitHub-Release v1.3.1)
