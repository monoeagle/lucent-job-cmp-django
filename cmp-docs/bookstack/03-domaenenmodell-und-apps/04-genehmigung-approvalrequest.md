...
`cmp/apps/approvals/models.py:8`, Tabelle `approval_rules`:
...
| `approver_role` | `CharField(20)`, `choices=UserRole.choices`, default `approver` | Rolle, die zur Entscheidung nötig ist |
...
`approver_role` war bis zur Migration `0002_alter_approvalrule_approver_role.py`
ein Freitext-Feld ohne `choices=` — ein Wert außerhalb der Rollenhierarchie hätte
die zugehörige Anfrage für niemanden mehr entscheidbar gemacht. Seit `0002` ist
das Feld auf die vier Werte aus `UserRole.choices` beschränkt
(`cmp/apps/approvals/models.py:20-22`); Details zur Absicherung in
[Kapitel 5.3](../05-genehmigungs-workflow/03-ein-und-mehrstufige-genehmigung.md).

## 3. Feldreferenz ApprovalRequest

`cmp/apps/approvals/models.py:32`, Tabelle `approval_requests`:
...
(`cmp/apps/approvals/services.py:26`) sammelt alle `ApprovalRule`-Zeilen mit
...
(`cmp/apps/orders/services.py:78-93`) bringt eine Order bis `submitted` und
stoppt dort; nichts im Anwendungscode prüft danach `needs_approval()` oder
erzeugt Genehmigungsanträge automatisch.

Auch `ApprovalService.approve()`/`reject()` — aufgerufen aus
`cmp/apps/approvals/views.py:36` und `:51` über die Genehmigungs-Queue — setzen
nur den Status der bereits existierenden `ApprovalRequest` und ggf. der `Order`
(`approved`/`rejected`), seit AP-22 zusätzlich vorgelagert über
`ApprovalService._load_pending` geprüft (Kapitel 5.4). Sie erzeugen selbst keine
neuen Anträge und stoßen auch kein Provisioning an (siehe
[Kapitel 3.5](05-provisioning-und-subscription.md)).
...
> Quelle: cmp/apps/approvals/models.py, cmp/apps/approvals/migrations/0002_alter_approvalrule_approver_role.py, cmp/apps/approvals/services.py, cmp/apps/approvals/views.py, cmp/apps/orders/services.py, cmp/apps/accounts/management/commands/seed.py — am Code geprüft 2026-07-22
