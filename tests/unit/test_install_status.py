...
ist: PostgreSQL/nginx/Dienste werden ueber Fakes bzw. MPP_PG_PREFIX gestellt.
"""
...
def test_status_service_aktiv(fake_systemctl):
    r = run_sh(
        ...
        env={"MPP_SYSTEMCTL": fake_systemctl, "FAKE_ACTIVE_UNITS": "mpp-web"},
    )

    assert r.returncode == 0, r.stderr
    assert r.stdout.strip() == "R|ok|mpp-web|aktiv"


def test_status_service_inaktiv(fake_systemctl):
    r = run_sh(
        "mpp_status_service mpp-web",
        env={"MPP_SYSTEMCTL": fake_systemctl, "FAKE_ACTIVE_UNITS": ""},
    )

    assert r.stdout.strip() == "R|fail|mpp-web|inaktiv"
...
def test_status_nginx_fehlend(tmp_path):
    r = run_sh("mpp_status_nginx", env={"MPP_NGINX": str(tmp_path / "weg")})
...
def test_status_nginx_vorhanden(tmp_path):
    ...
    r = run_sh("mpp_status_nginx", env={"MPP_NGINX": str(b)})
...
def test_status_postgres_zeigt_erkannte_variante(tmp_path, fake_systemctl):
    ...
    r = run_sh(
        "mpp_status_postgres",
        env={
            "MPP_PG_PREFIX": str(root),
            "MPP_SYSTEMCTL": fake_systemctl,
...
def test_status_postgres_fehlend(tmp_path, fake_systemctl):
    r = run_sh(
        "mpp_status_postgres",
        env={"MPP_PG_PREFIX": str(tmp_path), "MPP_SYSTEMCTL": fake_systemctl},
    )
...
def test_status_app_nicht_installiert(tmp_path):
    r = run_sh(f'mpp_status_app "{tmp_path}/gibtsnicht"')
...
def test_links_nutzen_fqdn_aus_der_env_datei(tmp_path):
    ...
    envfile.write_text("ALLOWED_HOSTS=mpp.intern\n")

    r = run_sh(f'mpp_status_links "{envfile}"')
...
def test_links_erfinden_keine_url_wenn_nichts_installiert_ist(tmp_path):
    ...
    URL stehen."""
    r = run_sh(f'mpp_status_links "{tmp_path}/gibtsnicht.env"')
