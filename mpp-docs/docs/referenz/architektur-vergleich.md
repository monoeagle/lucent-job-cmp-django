...
MPP existiert als zwei bewusst gegensätzliche Schwesterprojekte, die dasselbe
Self-Service-IT-Provisioning-Portal umsetzen. **MPP Django** (dieses Projekt,
`lucent-job-mpp-TDD-Django`) rendert HTML auf dem Server (**SSR**) — bewusst
*ohne* DRF, *ohne* React, *ohne* API-First. **lucent-job-MPP** liefert ein
...
| Dimension           | MPP Django (dieses Projekt · SSR)        | lucent-job-MPP (Schwester · API-First)       |
|---------------------|------------------------------------------|----------------------------------------------|
...
Die Präsentationsschicht von MPP Django lebt vollständig im Server; es existiert
keine JSON-API — der Kontrakt ist Template ↔ Context.

| Merkmal                          | MPP Django (SSR)                        | lucent-job-MPP (API-First)          |
|----------------------------------|-----------------------------------------|-------------------------------------|
...
| Ebene           | MPP Django (SSR)                     | lucent-job-MPP (API-First)            |
|-----------------|--------------------------------------|---------------------------------------|
...
| Dimension     | MPP Django (SSR)                        | lucent-job-MPP (API-First)                  |
|---------------|------------------------------------------|---------------------------------------------|
...
| Tests         | **317**                                 | ≈ **878** (771 Backend / 107 Frontend)      |

## Kern-Trade-offs

### SSR (MPP Django · Django + HTMX)
...
### API-First (lucent-job-MPP · Flask + React)
...
**MPP Django** treibt den SSR-Weg — der Server rendert HTML, Session-Auth,
Django Admin, HTMX für punktuelle Dynamik, bewusst *ohne* DRF/API-Layer.
**lucent-job-MPP** treibt den API-First/SPA-Weg — JSON-Backend, typisierte
