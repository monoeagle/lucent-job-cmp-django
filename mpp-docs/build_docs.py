#!/usr/bin/env python3
"""
build_docs.py — MPP Django Doku-Pipeline.
...
Nutzung:
    ...
    python3 build_docs.py --serve --port 8048
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="MPP Django Docs — voller Build inkl. Mermaid + Activity-JSON",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument("--serve",       action="store_true", help="Nach Build HTTP-Server starten")
    parser.add_argument("--port",        type=int, default=5063, help="Port fuer --serve (Default: 5063)")
    ...
    parser.add_argument("--no-mermaid",  action="store_true", help="Mermaid-Pipeline (extract + render) ueberspringen")
